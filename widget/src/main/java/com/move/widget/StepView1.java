...
public class StepView1 extends ViewGroup {
    ...
    public StepView1(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public StepView1(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        // 读取自定义属性
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.StepView1);

        hSpace = a.getDimensionPixelOffset(R.styleable.StepView1_sv1_horizontal_space, dpToPx(10));
        vSpace = a.getDimensionPixelOffset(R.styleable.StepView1_sv1_vertical_space, dpToPx(2));

        textSize = a.getInt(R.styleable.StepView1_sv1_text_size, 16);
        unSelectedTextColor = a.getColor(R.styleable.StepView1_sv1_text_color, Color.GREEN);
        selectedTextColor = a.getColor(R.styleable.StepView1_sv1_text_selected_color, Color.GRAY);

        circleBorderWidth = a.getDimensionPixelOffset(R.styleable.StepView1_sv1_circle_border_width, dpToPx(4));
        circleRadius = a.getDimensionPixelOffset(R.styleable.StepView1_sv1_circle_radius, dpToPx(8));
        unSelectedCircleColor = a.getColor(R.styleable.StepView1_sv1_circle_color, Color.GREEN);
        selectedCircleColor = a.getColor(R.styleable.StepView1_sv1_circle_selected_color, Color.GRAY);

        lineHeight = a.getDimensionPixelOffset(R.styleable.StepView1_sv1_line_height, dpToPx(2));
        lineMargin = a.getDimensionPixelOffset(R.styleable.StepView1_sv1_line_margin, dpToPx(4));
        unSelectedLineColor = a.getColor(R.styleable.StepView1_sv1_line_color, Color.GREEN);
        selectedLineColor = a.getColor(R.styleable.StepView1_sv1_line_selected_color, Color.GRAY);

        progress = a.getInt(R.styleable.StepView1_sv1_progress, -1);

        int resourceId = a.getResourceId(R.styleable.StepView1_sv1_data, -1);
        if (resourceId != -1) {
